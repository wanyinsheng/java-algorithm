 ...
 * 
 * 输入： ghosts = [[1, 0], [0, 3]] target = [0, 1] 输出：true 解释：
...
public class EscapeGhosts {
	...
	static boolean escapeGhosts(int[][] ghosts, int[] target) {
		int people = Math.abs(target[0]) + Math.abs(target[1]);// 人到目的地的距离,注意用绝对值
		int gho = Integer.MAX_VALUE;
		int persent = 0;
		for (int i = 0; i < ghosts.length; i++) {
			persent = Math.abs(target[0] - ghosts[i][0]) + Math.abs(target[1] - ghosts[i][1]);// 第i个幽灵距目的地的距离
			if (persent < gho) {
				gho = persent;
			}
		}
